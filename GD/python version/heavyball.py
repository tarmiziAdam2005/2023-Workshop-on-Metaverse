# -*- coding: utf-8 -*-
"""
Created on Sat Dec  9 18:53:50 2023
...
def tikhonov_hb(g, mu, lamda, nit, tol, n):
    x_k0 =g.copy()
    x_k1=x_k0.copy()
    x_k2=x_k0.copy()
    beta=0.01
    ...
    for k in range(nit):
        ...
        x_k2 = y_k2 - alpha *( x_k1 - g + 2 * lamda * D.T * D * x_k1)
        
        x_k0 = x_k1.copy()
        x_k1 = x_k2.copy()
        
        RelErr[k] = np.linalg.norm(x_k2 - x_old, "fro") / np.linalg.norm(x_k2, "fro")
...
tol = 1e-10
lamda = 1
v, Err = tikhonov_hb(y, mu, lamda, nit, tol, n) 
...
plt.axis('tight')
plt.grid()
plt.legend(['HeavyBall'], loc='best', fontsize=12)
